...
class Client(object):
    ...
    def get_game_players(self, id):
        self.socket.send("%s %s%s" % (ProtocolExtended.GAME_PLAYERS, id, Protocol.EOL))

        print("The following players are in game %s active:" % id)
        self.print_response()

    def get_game_board(self, id):
        self.socket.send("%s %s%s" % (ProtocolExtended.GAME_BOARD, id, Protocol.EOL))

        print("The board for game %s looks like:" % id)
